...
export const createSection = createAsyncThunk(
  'section/create',
  async (section: Section, thunkAPI) => {
    try {
      const response = await addSection(section)
      return { currentSection: response }
    } catch (e) {
      return thunkAPI.rejectWithValue(
        'データ作成（section/create）に失敗しました。\n' + e
      )
    }
  }
)

export const changeSection = createAsyncThunk(
  'section/change',
  async (section: Section, thunkAPI) => {
    try {
...
export const sectionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(changeSection.pending, (state) => ({
      ...state,
      status: 'loading',
    }))
    builder.addCase(changeSection.fulfilled, (state, action) => {
      const { section } = action.payload
      const sections = state.sections.map((x) =>
        x.id === section.id ? section : x
      )
      return { ...state, status: 'idle', sections }
    })
    builder.addCase(changeSection.rejected, (state) => ({
      ...state,
      status: 'failed',
    }))
    builder.addCase(destroySection.pending, (state) => ({
      ...state,
      status: 'loading',
    }))
    ...
    builder.addCase(destroySection.rejected, (state) => ({
      ...state,
      status: 'failed',
    }))
  },
