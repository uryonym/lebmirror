...
export const destroyPage = createAsyncThunk(
  'page/destroy',
  async (pageId: string, thunkAPI) => {
    try {
      await deletePage(pageId)
      return { pageId }
    } catch (e) {
      return thunkAPI.rejectWithValue(
        'データ削除（page/destroy）に失敗しました。\n' + e
      )
    }
  }
)
...
export const pageSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(changePage.rejected, (state) => ({
      ...state,
      status: 'failed',
    }))
    builder.addCase(destroyPage.pending, (state) => ({
      ...state,
      status: 'loading',
    }))
    ...
    builder.addCase(destroyPage.rejected, (state) => ({
      ...state,
      status: 'failed',
    }))
  },
