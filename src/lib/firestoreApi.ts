...
const noteConverter: FirestoreDataConverter<Note> = {
  ...
  fromFirestore(
    snapshot: QueryDocumentSnapshot<Note>,
...
const sectionConverter: FirestoreDataConverter<Section> = {
  ...
  fromFirestore(
    snapshot: QueryDocumentSnapshot<Section>,
    options?: SnapshotOptions
  ): Section {
    const data = snapshot.data(options)
    const createdAt =
...
const pageConverter: FirestoreDataConverter<Page> = {
  ...
  fromFirestore(
    snapshot: QueryDocumentSnapshot<Page>,
    options?: SnapshotOptions
  ): Page {
    const data = snapshot.data(options)
    const createdAt =
...
export const getSections = async (noteId: string) => {
  const collRef = collection(firebaseDb, 'sections').withConverter(
    sectionConverter
  )
